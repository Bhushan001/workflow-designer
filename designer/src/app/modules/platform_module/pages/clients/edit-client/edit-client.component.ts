  ...
  loadClient(): void {
    ...
    this.clientService.getClientById(this.clientId).subscribe({
      ...
      error: (err) => {
        ...
        this.error = err.error?.message || 'Failed to load client. Please try again.';
        this.loadingClient = false;
      }
    });
  }

  populateForm(client: any): void {
  ...
  onSubmit(): void {
    if (this.clientForm.valid && this.clientId) {
      ...
      this.clientService.updateClient(this.clientId, clientData).subscribe({
        next: (response) => {
          this.loading = false;
          this.toastService.showToast('success', 'Client Updated', `Client "${clientData.name}" has been updated successfully.`);
          // Navigate back to clients list
          this.router.navigate(['/platform/clients']);
        },
        error: (err) => {
          console.error('Error updating client:', err);
          const errorMsg = extractErrorMessage(err) || 'Failed to update client. Please try again.';
          this.error = errorMsg;
          this.toastService.showToast('danger', 'Client Update Failed', errorMsg);
          this.loading = false;
        }
      });
    } else {
